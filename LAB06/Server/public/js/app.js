$(document).ready(function(){
	...
	socket.on('actualizado', function(data){
		return alert('Registro actualizado: ' + data.first_name + ' ' + data.last_name);
	});
	...
	var fill = function(data){
		if($('#'+data._id).length==0){
			...
			$row.append('<td>'+data.active+'</td>');
			$row.append('<td><button class="btn btn-success btn-sm" name="btnAct">Actualizar</td>');
			$row.append('<td><button class="btn btn-success btn-sm" name="btnEli">Eliminar</td>');
			$row.data('data',data);
			...
			$row.find('[name=btnAct]').click(function(){
				...
				if(data.active.toString() == 'true') $('#active').val('true');
			...
			$row.find('td:eq(6)').html(data.active.toString());
		}
	};

	function blinker(_id) {
		var $row = $('#' + _id);

		$row.find('td:eq(1)').addClass('blinker');
		$row.find('td:eq(2)').addClass('blinker');
		$row.find('td:eq(3)').addClass('blinker');
		$row.find('td:eq(4)').addClass('blinker');
		$row.find('td:eq(5)').addClass('blinker');
		$row.find('td:eq(6)').addClass('blinker');

		setTimeout(function(){
			$row.find('td:eq(1)').removeClass('blinker');
			$row.find('td:eq(2)').removeClass('blinker');
			$row.find('td:eq(3)').removeClass('blinker');
			$row.find('td:eq(4)').removeClass('blinker');
			$row.find('td:eq(5)').removeClass('blinker');
			$row.find('td:eq(6)').removeClass('blinker');
		}, 1000*8);
